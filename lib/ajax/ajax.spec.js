...
describe('"AJAX" module:', function() {
  ...
  describe('ajax.request', function() {
    ...
    describe('REST requests', function() {
      it('should successfully execute a GET request', function() {
        return expect(ajax.request('/get', { method: 'get' })).to.eventually.have.property('status', 'success');
      });

      it('should successfully execute a POST request', function() {
        return Promise.all([
          expect(ajax.request('/post', {
            method: 'post',
            data: formData
          })).to.eventually.have.property('status', 'success'),
          expect(ajax.request('/post', { method: 'post', data: string })).to.eventually.have.property('status', 'success'),
          expect(ajax.request('/post', { method: 'post', data: obj })).to.eventually.have.property('status', 'success')
        ]);
      });

      it('should successfully execute a PUT request', function() {
        return Promise.all([
          expect(ajax.request('/put', { method: 'put', data: formData })).to.eventually.have.property('status', 'success'),
          expect(ajax.request('/put', { method: 'put', data: string })).to.eventually.have.property('status', 'success'),
          expect(ajax.request('/put', { method: 'put', data: obj })).to.eventually.have.property('status', 'success')
        ]);
      });

      it('should successfully execute a DELETE request', function() {
        return Promise.all([
          expect(ajax.request('/delete', {
            method: 'delete',
            data: formData
          })).to.eventually.have.property('status', 'success'),
          expect(ajax.request('/delete', {
          ...
          expect(ajax.request('/delete', { method: 'delete', data: obj })).to.eventually.have.property('status', 'success')
        ]);
      });

      it('should successfully call the progress event of a POST request', function(done) {
        const spy = sinon.spy();
        ajax.request('/post', { method: 'post', progress: spy })
            .then(
            () => {
    ...
    it('should fail (Promise.catch) when errors arise', function() {
      return Promise.all([
        expect(ajax.request('/notmapped', { method: 'get' })).to.eventually.be.rejected,
        expect(ajax.request('/malformedjson', { method: 'get' })).to.eventually.be.rejected
      ]);
    });
  });
  ...
  describe('shortcut methods:', function() {
    ...
    // --- DELETE ---
    describe('ajax.del', function() {
      it('should perform a request using the DELETE method', function() {
        const data = { field: 'test' };

        return Promise.all([
          expect(ajax.del('/delete', { data: data })).to.eventually.have.property('status', 'success'),
          expect(ajax.del('/delete', data)).to.eventually.have.property('status', 'success'),
          expect(ajax.del('/post', data)).to.eventually.be.rejected
        ]);
      });
    });
  ...
  describe('ajax.getCSRF', function() {
    ...
    it('should successfully get CSRF token', function(done) {
      ...
      // Get new CSRF token
      ajax.getCSRF(config).then((csrf) => {
        expect(csrf).to.equal('csrftoken');
        expect(config.csrf).to.equal('csrftoken');
        done();
      })
      .catch(done);
    });
    ...
    describe('REST with CSRF enabled', function() {
      ...
      it('should successfully execute a DELETE request', function() {
        return expect(ajax.del('/delete', conf)).to.eventually.have.property('status', 'success');
      });
