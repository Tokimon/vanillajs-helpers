...
const event = {
  on: (elm, eventName, handler) => { elm.addEventListener(eventName, handler, true); },
  off: (elm, eventName, handler) => { elm.removeEventListener(eventName, handler, true); },
  trigger: (elm, eventName, data) => { elm.dispatchEvent(createEvent(eventName, data)); }
};
